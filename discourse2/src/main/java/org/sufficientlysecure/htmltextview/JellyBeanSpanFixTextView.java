...
/**
 * <p/>
 * A {@link android.widget.TextView} that insert spaces around its text spans where needed to prevent {@link IndexOutOfBoundsException} in
 * {@link #onMeasure(int, int)} on Jelly Bean.
 * <p/>
 * When {@link #onMeasure(int, int)} throws an exception, we try to fix the text by adding spaces around spans, until it works again. We
 * then try removing some of the added spans, to minimize the insertions.
 * <p/>
