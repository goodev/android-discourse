...
public abstract class CompositeCursorAdapter extends BaseAdapter {
    ...
    /**
     * Registers a partition. The cursor for that partition can be set later. Partitions should be added in the order they are supposed to
     * appear in the list.
     */
    ...
    /**
     * Returns the overall number of item view types across all partitions. An implementation of this method needs to ensure that the
     * returned count is consistent with the values returned by {@link #getItemViewType(int, int)}.
     */
    ...
    protected View getHeaderView(int partition, Cursor cursor, View convertView, ViewGroup parent) {
        View view = convertView != null ? convertView : newHeaderView(mContext, partition, cursor, parent);
    ...
    /**
     * Enable or disable data change notifications. It may be a good idea to disable notifications before making changes to several
     * partitions at once.
     */
