...
/**
 * A class that wraps up remote image loading requests using the Volley library combined with a memory cache. An single instance of this
 * class should be created once when your Activity or Fragment is created, then use {@link #get(String, android.widget.ImageView)} or one of
 * the variations to queue the image to be fetched and loaded from the network. Loading images in a {@link android.widget.ListView} or
 * {@link android.widget.GridView} is also supported but you must store the {@link com.android.volley.Request} in your ViewHolder type class
 * and pass it into loadImage to ensure the request is canceled as views are recycled.
 */
public class ImageLoader extends com.android.volley.toolbox.ImageLoader {
     ...
     */
    private static final int DEFAULT_DISK_USAGE_BYTES = 6 * 1024 * 1024;
    ...
    /**
     * Sets a {@link android.graphics.Bitmap} to an {@link android.widget.ImageView} using a fade-in animation. If there is a
     * {@link android.graphics.drawable.Drawable} already set on the ImageView then use that as the image to fade from. Otherwise fade in
     * from a transparent Drawable.
     */
    ...
    public ImageContainer getForImageSpan(UrlImageGetter imageGetter, String requestUrl, TextView imageView, ImageListener listener) {
    ...
    // TODO 在ListView滚动的时候 暂停 然后再启动 会导致 com.android.volley.RequestQueue.start() 函数
    // 反复调用， 并反复初始化 本地缓存， 导致加载图片速度慢
