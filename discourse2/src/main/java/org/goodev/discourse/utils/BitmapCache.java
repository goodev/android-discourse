...
import static org.goodev.discourse.utils.LogUtils.LOGD;
import static org.goodev.discourse.utils.LogUtils.makeLogTag;
...
public class BitmapCache implements ImageLoader.ImageCache {
    ...
    /**
     * Find and return an existing BitmapCache stored in a {@link RetainFragment}, if not found a new one is created using the supplied
     * params and saved to a {@link RetainFragment}.
     *
     * @param fragmentManager The fragment manager to use when dealing with the retained fragment.
     * @param fragmentTag     The tag of the retained fragment (should be unique for each memory cache that needs to be retained).
     * @param memCacheSize    Memory cache size in KB.
     */
    ...
    /**
     * Sets the memory cache size based on a percentage of the max available VM memory. Eg. setting percent to 0.2 would set the memory
     * cache to one fifth of the available memory. Throws {@link IllegalArgumentException} if percent is < 0.05 or > .8. memCacheSize is
     * stored in kilobytes instead of bytes as this will eventually be passed to construct a LruCache which takes an int in its constructor.
